...
if(isset($_POST['submit'])){
	...
	$GetOwnerQuery = "SELECT * from owner WHERE username='$OwnerName'";
	$result = $conn->query($GetOwnerQuery);
	$row = $result->fetch_assoc();
	if($Password!= $row['password'] || $Name!= $row['Fullname'] || $email!= $row['email'] || $PhoneNum!= $row['phone']){
		$UpdateProfitQuery = "UPDATE owner SET password='$Password' , Fullname='$Name' ,email='$email' ,phone='$PhoneNum' WHERE username='$OwnerName'";
					...
					<form action="" method="Post" onsubmit="return Volunteer_Validate()" enctype="multipart/form-data">
						<div>
							<?php
								...
								$GetUserQuery = "SELECT * from owner WHERE username='$UserName'";
